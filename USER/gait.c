...
// L_onestep=25;一个周期内行走的距离、 W_onestep=0;转弯角度，左负右正、  H_onestep =10;抬腿高度。
...
void movement_tripod(void)
{
  ...
  float alpha;
	...
	{	
		...
		z[1]=-L3+H_onestep-H_onestep*(sqrt(L_onestep*L_onestep+W_onestep*W_onestep)*4*s/T*3/4- sqrt(L_onestep*L_onestep+W_onestep*W_onestep)/8*3)*
		...
		if(s>=T/2&&s<T*3/4)
		{
			...
			z[0]=-L3+H_onestep-H_onestep*(sqrt(L_onestep*L_onestep+W_onestep*W_onestep)*4*(s-T/2)/T*3/4-sqrt(L_onestep*L_onestep+W_onestep*W_onestep)/8*3)*
...
/*hold the pillar*/
//	KMGecko.StartAngle[RF_J3] = -2; //52
//	KMGecko.StartAngle[RF_J2] = 2; //26//large figure turning to the inside
//	KMGecko.StartAngle[RF_J1] = 0; //// large figure turning to the inside
//   //number 2
//	KMGecko.StartAngle[RH_J3] = 0;//-10//-25;	
//	KMGecko.StartAngle[RH_J2] = 0;//50;	small figure clockwise
//	KMGecko.StartAngle[RH_J1] = -8;//30;small figure turning to the clw
//  //number 0
//	KMGecko.StartAngle[LF_J3] = -4;//-10
//	KMGecko.StartAngle[LF_J2] = 0;//-13 small figure turning to the inside
//	KMGecko.StartAngle[LF_J1] = -8;//-58 small figure turning to the inside
//	//number 1
//	KMGecko.StartAngle[LH_J3] = -6;//10///-30;//原值为-30
//	KMGecko.StartAngle[LH_J2] = 0;//0 large figure turning to the inside
//	KMGecko.StartAngle[LH_J1] = 8;//;small figure turning to the inside
}

...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
    if(htim==(&TIM5_Handler))
    {
			...
			//²½Ì¬¡¢ÖÜÆÚ¡¢ĞĞ³Ì¡¢×ªÍä½Ç¶È±ØĞëÒ»¸öÖÜÆÚ½áÊøÖ®ºó²Å¿ÉÒÔ½øĞĞÉèÖÃ	
			if(s>T){			
			   //ÉèÖÃ²½Ì¬±êÖ¾ 0ºÍfalseÎª¶Ô½Ç 1ºÍtrueÎªÈı½Ç
				if(receiveSetG==0)
					{
						gait_flag=0;	
					}	
					...
					//ÖÜÆÚ¡¢ĞĞ³Ì¡¢×ªÍä½Ç¶ÈÉèÖÃ
					T=receiveSetT;
					L_onestep=receiveSetL;
					W_onestep=receiveSetW;				
		      }
			
			//ÓÉÔË¶¯±êÖ¾¾ö¶¨»úÆ÷ÈËÊÇ·ñÔË¶¯
			if(recMotion_flag==1)
			{
		   //ÓÉ²½Ì¬±êÖ¾ÔËĞĞÏàÓ¦º¯Êı
			 if(gait_flag==1)	
			 	 {
						air_control_tripod();
						movement_tripod();	
